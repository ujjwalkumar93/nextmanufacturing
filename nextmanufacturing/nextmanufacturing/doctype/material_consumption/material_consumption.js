...
frappe.ui.form.on('Material Consumption', {
    ...
    make_pick_list: function(frm){
        //console.log("frm.doc.name: ", frm.doc.name)
        //var items = [{"item_code": "RM-00245"}]
        frappe.new_doc("Pick List", {"material_consumption": frm.doc.name, "purpose": "Material Transfer"})
    },
    get_items_from_pick_list: function(frm){
        ...
        frappe.prompt(
            ...
            function(data) {
                frm.call({
                    method: "nextmanufacturing.nextmanufacturing.doctype.material_consumption.material_consumption.add_pick_list_item",
                    args: {
...
function get_available_qty_data(frm,line_obj){
    frappe.call({
        method: "nextmanufacturing.nextmanufacturing.doctype.material_consumption.material_consumption.get_available_qty_data",
        args: {
            line_id: line_obj.name,
            company: frm.doc.company,
            item_code: line_obj.item_code,
            warehouse: line_obj.t_warehouse,
